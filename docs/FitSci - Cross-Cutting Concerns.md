...
### Why
A re-evaluation of the same paper through the same model digest must hit cache, not re-run inference. See [`audit-gemma4-selection.md §6`](./internal/audit/audit-gemma4-selection.md).
...
- **Raw paper text** — never persisted (Extract & Discard pattern, [`audit-database.md §1`](./internal/audit/audit-database.md)).
...
- Audits live in `docs/internal/audit/` and are versioned at the date of the audit; never edited in place after the fact (corrections go in a follow-up audit).
